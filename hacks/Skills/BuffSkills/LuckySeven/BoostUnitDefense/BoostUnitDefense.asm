...
CheckCharacter:
    mov     r1,r0               
    mov     r0,#0x17            @vanilla instruction 1 - get the defense bit
    ldsb    r0,[r4,r0]          @vanilla instruction 2 - load the defense value
    ...
    ldr     r3,BoostUnitDefenseID   @load the ID value we have defined
...
CheckBitFlag:
    ...
    cmp     r2,#0x5             @compare it to the value we're using to represent the defense stat
    beq     BoostDefense        @if there's match then branch to boost the unit's defense
    b       End                 @else branch to the end

BoostDefense:
    add     r1,#7               @add 7 to the defense boost
    b       End

End:
    add     r0,r0,r1            @vanilla instruction 4 - add the defense and boost together for the final total
    pop     {r2,r5}
    pop     {r3}
...
.ltorg
.align
BoostUnitDefenseID:             @refer to the value defined in the event file
